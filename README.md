...
![Platform](https://img.shields.io/badge/Platform-JVM%20%7C%20JS%20%7C%20WasmJS-orange.svg)
...
项目采用 MVI (Model-View-Intent) 架构模式，确保单向数据流和状态管理的清晰性。
...
#### 1. Base 层
- **BaseIntent**: 所有 Intent 的基类
- **BaseUiState**: 所有 UiState 的基类
- **BaseEffect**: 所有副作用的基类
- **BaseViewModel**: ViewModel 基类，处理状态管理

#### 2. Domain 层
- **ToolCategory**: 工具分类枚举
- **ToolCommand**: 工具命令数据类
- **CommandResult**: 命令执行结果
- **ExecuteCommandUseCase**: 命令执行用例

#### 3. Platform 层（跨平台）
- **CommandExecutor**: 命令执行器（expect/actual）
- **DeviceManager**: 设备管理器（expect/actual）
- **FileChooser**: 文件选择器（expect/actual）
- **SystemPaths**: 系统路径工具（expect/actual）

#### 4. UI 层
- **Sidebar**: 侧边栏导航
- **ToolsScreen**: 工具界面
- **DeviceManagerScreen**: 设备管理界面
- **TerminalLog**: 终端日志组件
- **FileInputField**: 文件输入组件
